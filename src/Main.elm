...
init _ =
  (Model "" "" False,
  Random.generate GenerateWord (Random.int 0 ((Array.length wordsList)-1)))
  

-- UPDATE
type Msg = Change String  | GenerateWord Int

update : Msg -> Model -> (Model, Cmd Msg)
update msg model =
  case msg of 
    Change guess -> if guess== model.toGuess then (Model "" "" True, Cmd.none) else (Model guess model.toGuess False, Cmd.none)
    GenerateWord newInt ->
      ( Model ""  (grabString (Array.get (newInt) wordsList)) False
      , Cmd.none
      )
...
view model =
      ...
      h1 [][text ("to guess : " ++ model.toGuess)],
        p[] [text "Definition"],
        input [ placeholder "Take a guess", value model.guess, onInput Change] []
      ]
